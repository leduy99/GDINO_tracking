...
def run(args):
  ...
  with open(args.source, "r") as f:
    for line in f:
        ...
        name_parts = folder_set[0].split('/')
        folder_name = name_parts[0]
        query_name = name_parts[1]
        ...
        print(f'start tracking {folder_name}/{query_name}')

        src_folder = os.path.join('/content/GenericMOT_JPEG_Sequence', folder_name, 'img1')
        dest_folder = os.path.join(args.save_dir, 'img', folder_name, query_name)
        if not os.path.exists(dest_folder):
            os.makedirs(dest_folder)

        # Create text path and clear content in text file
        parent_txt_path = os.path.join(args.save_dir, 'text', args.feature_mode, folder_name)
        if not os.path.exists(parent_txt_path):
            os.makedirs(parent_txt_path)

        txt_path = os.path.join(parent_txt_path, query_name + '.txt')
